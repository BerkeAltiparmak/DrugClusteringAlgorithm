...
def visualize_graph(graph: graph_creator.Graph,
    ...
    colours = [USER_COLOUR for kind in kinds]

    # my addition so that each cluster has their own associated color:
    border_colors = [('rgb(' + str((int(kind) * 3) % 256) + ', ' + str((int(kind) * 5) % 256)
                      + ', ' + str((int(kind) * 7) % 256) + ')') for kind in kinds]
    line_colors = [('rgb(' + str((int(kind) * 3) % 256) + ',' + str((int(kind) * 5) % 256)
                    + ',' + str((int(kind) * 7) % 256) + ')') for kind in kinds]
    ...
    trace3 = Scatter(x=x_edges,
                     ...
                     mode='lines',
                     name='edges',
                     line=dict(color=LINE_COLOUR, width=1),  #DOESN'T WORK
                     hoverinfo='none',
                     )
...
def visualize_graph_clusters(graph: graph_creator.Graph, clusters: list[set],
                             layout: str = 'spring_layout',
                             max_vertices: int = 5000,
